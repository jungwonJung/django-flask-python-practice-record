...
class ExpenseListAPIView(ListCreateAPIView): # ListCreateAPIView   GET, POST API 기능 생성
    serializer_class = ExpenseSerializer # 사용할 직렬화 class 설정 
    query_set = Expense.objects.all()  # .models.py 에서 Expense class 의 저장된 objects 전부를 query_set 으로 지정한다
...
class ExpenseDetailAPIView(RetrieveUpdateDestroyAPIView):   # RetrieveUpdateAPIView 를 사용하면 mixin 과 비슷한기능으로 GET, PUT,PATCH , DELETE 기능 생성
